...
const FingerprintUploader = ({ onVerificationComplete, disabled = false, mode = 'verify', voterId = 'DEMO_VOTER' }) => {
  ...
  const maxFileSize = 10 * 1024 * 1024; // 10MB for TIF files (they can be larger)
  ...
  const handleFile = async (file) => {
    ...
    for (const pattern of suspiciousPatterns) {
      if (filenameLower.includes(pattern)) {
        setVerificationStatus({
          type: 'error',
          message: `File name contains suspicious pattern '${pattern}'. Only fingerprint image files are allowed.`
        });
        return;
      }
    }
    ...
    console.log('✅ File validation passed, proceeding with verification...');
    ...
    // Auto-enroll fingerprint immediately after file selection
  ...
  const handleVerify = async (fileToVerify = null) => {
    ...
    console.log('🔍 Starting fingerprint verification...', {
      ...
      fileSize: file.size,
      fileType: file.type,
      voterId: voterId || 'TEMP_VALIDATION',
      mode: mode
    });
    ...
    try {
      // Use enhanced biometric service for robust verification/enrollment
      const formData = new FormData();
      formData.append('fingerprint', file);
      // Use voterId if available, otherwise use a temporary ID for validation
  ...
  return (
    <div className="space-y-4">
      {/* Upload Area */}
      <div
        className={`fingerprint-upload-area ${dragActive ? 'dragover' : ''} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Drag and drop your fingerprint image here, or click to browse
              </p>
      ...
      {selectedFile && isVerifying && (
        <motion.div
          ...
          className="flex items-center justify-center space-x-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg"
        >
          <svg
